...
public sealed class LeaveService
{
    ...
    private readonly Dictionary<string, LeaveBalance> _balances;
    ...
    public LeaveService(DemoData data, OaDbContext? db = null, IWorkCalendar? calendar = null, NotificationService? notifications = null, FileService? files = null, IProcessRouter? processRouter = null, FlowInstanceService? flowInstances = null, FlowCopyService? copyRecipients = null)
    {
        ...
        _balances = data.Employees.ToDictionary(item => item.Id, item => LoadBalance(item));
    }
    ...
    public LeaveBalance GetBalance(Employee actor) => _balances[actor.Id];
    ...
    public ServiceResult<LeaveRequest> Submit(Employee actor, Guid id)
    {
        ...
        {
            var balance = _balances[actor.Id];
            if (balance.Available < item.Days)
                return ServiceResult<LeaveRequest>.Failure("假期余额不足。", "LEAVE_001");
            _balances[actor.Id] = balance with { Frozen = balance.Frozen + item.Days };
            PersistBalance(actor.Id, item.Type);
        }
        ...
        if (item.Tasks.OrderBy(task => task.Sequence).FirstOrDefault() is { } firstTask)
            notifications?.Create(firstTask.AssigneeId, "TODO_CREATED", "新增请假审批待办", $"{item.ApplicantName} 提交了 {item.Number}", "LeaveRequest", item.Id);
        return ServiceResult<LeaveRequest>.Success(item);
    }
    ...
    public ServiceResult<bool> Delete(Employee actor, Guid id)
    {
        ...
        if (db is not null)
        {
            ...
            db.LeaveRequests.Where(request => request.Id == item.Id).ExecuteDelete();
            db.SaveChanges();
        }
    ...
    public ServiceResult<LeaveRequest> Approve(Employee actor, Guid taskId, string? comment)
    {
        ...
        Persist(item);
        var activatedCopies = item.Status == LeaveStatus.Completed ? copyRecipients.Activate("Leave", item.Id, item.Status.ToString()) : [];
        Audit(actor, "LEAVE_APPROVED", item, comment?.Trim() ?? "同意请假");
        if (item.Status == LeaveStatus.Completed)
        {
            notifications?.Create(item.ApplicantId, "LEAVE_COMPLETED", "请假申请已通过", $"{item.Number} 已全部审批通过", "LeaveRequest", item.Id);
            foreach (var recipient in activatedCopies) notifications?.Create(recipient.Id, "FLOW_COPY", "请假抄送事项已完成", $"{item.ApplicantName} 的 {item.Number} 已审批完成", "LeaveRequest", item.Id);
        }
            ...
            notifications?.Create(next.AssigneeId, "TODO_CREATED", "新增请假审批待办", $"{item.Number} 等待你审批", "LeaveRequest", item.Id);
        return ServiceResult<LeaveRequest>.Success(item);
    }
    ...
    public ServiceResult<LeaveRequest> Reject(Employee actor, Guid taskId, string comment)
    {
        ...
        notifications?.Create(item.ApplicantId, "LEAVE_REJECTED", "请假申请已驳回", $"{item.Number} 已被驳回：{comment.Trim()}", "LeaveRequest", item.Id);
        return ServiceResult<LeaveRequest>.Success(item);
    }
    ...
    public ServiceResult<LeaveRequest> Transfer(Employee actor, Guid taskId, TransferTaskRequest request)
    {
        ...
        notifications?.Create(assignee.Id, "TODO_TRANSFERRED", "请假审批待办已转办给你", $"{item.Number}：{request.Comment.Trim()}", "LeaveRequest", item.Id);
        return ServiceResult<LeaveRequest>.Success(item);
    }
    ...
    public ServiceResult<LeaveRequest> Withdraw(Employee actor, Guid id)
    {
        ...
        Persist(item);
        var activatedCopies = copyRecipients.Activate("Leave", item.Id, item.Status.ToString());
        Audit(actor, "LEAVE_WITHDRAWN", item, "撤回请假申请");
        foreach (var task in item.Tasks.Where(task => task.AssigneeId != actor.Id)) notifications?.Create(task.AssigneeId, "LEAVE_WITHDRAWN", "请假申请已撤回", $"{item.Number} 已被申请人撤回", "LeaveRequest", item.Id);
        foreach (var recipient in activatedCopies) notifications?.Create(recipient.Id, "FLOW_COPY", "请假抄送事项已撤回", $"{item.ApplicantName} 的 {item.Number} 已撤回", "LeaveRequest", item.Id);
        return ServiceResult<LeaveRequest>.Success(item);
    }
    ...
    private void ReleaseFrozenToUsed(Employee actor, LeaveRequest item)
    {
        if (item.Type is not (LeaveType.Annual or LeaveType.CompTime)) return;
        var balance = _balances[actor.Id];
        _balances[actor.Id] = balance with { Frozen = balance.Frozen - item.Days, Used = balance.Used + item.Days };
        PersistBalance(actor.Id, item.Type);
    }

    private LeaveBalance LoadBalance(Employee employee)
    {
        var entitled = employee.CumulativeWorkYears switch { < 1 => 0m, < 10 => 5m, < 20 => 10m, _ => 15m };
        if (db is null) return new LeaveBalance(entitled, 0m, 0m);
        var record = db.LeaveBalances.AsNoTracking().SingleOrDefault(x => x.TenantId == TenantId && x.UserId == employee.Id && x.LeaveType == (int)LeaveType.Annual);
        return record is null ? new LeaveBalance(entitled, 0m, 0m) : new LeaveBalance(record.Entitled, record.Frozen, record.Used);
    }
    ...
    private void PersistBalance(string employeeId, LeaveType type)
    {
        if (db is null || type is not (LeaveType.Annual or LeaveType.CompTime)) return;
        var balance = _balances[employeeId];
        var record = db.LeaveBalances.SingleOrDefault(x => x.TenantId == TenantId && x.UserId == employeeId && x.LeaveType == (int)type);
        if (record is null) { record = new LeaveBalanceRecord { TenantId = TenantId, UserId = employeeId, LeaveType = (int)type }; db.LeaveBalances.Add(record); }
        record.Entitled = balance.Entitled; record.Frozen = balance.Frozen; record.Used = balance.Used;
        db.SaveChanges();
    }
