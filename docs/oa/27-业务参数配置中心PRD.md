...
- **预算标准与超标计算口径**：
  - **允许预算上限** = `(住宿每晚限额 + 每日餐补标准) * 出差天数 * 出行人数 (1 + 同行人数)`。
  - **交通工具标准**（如“高铁二等座/飞机经济舱”）：作为定性合规标准，展示于申请端与审批端。由于长途交通票价随班次、淡旺季浮动较大，预估总预算超过允许预算上限即触发超标判定。
  - **超标闭环**：预估总预算超出允许预算上限时，单据标记为 `IsOverStandard = true`，申请人必须填写 `OverStandardReason`（超标原因，如包含机票预订、会期酒店浮动等），否则提交阻断（错误码 `TRAVEL_OVER_STANDARD`）；详情页与审批页完整展示标准上限、超标金额与填报原因。
