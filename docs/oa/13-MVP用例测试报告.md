...
本报告覆盖当前已实现的 PostgreSQL 用户/账号/部门树/岗位/人事档案与生命周期、考勤班次/打卡/异常申诉/月报、劳动合同/续签/预警派发、角色/权限、短期 JWT、TOTP MFA、刷新令牌轮换、服务端会话撤销、登录失败锁定、用户与组织架构管理、权限路由与服务端授权，以及请假、报销、流程实例与永久轨迹、审批委托、流程抄送、通知、工作日历、附件、幂等和持久化能力。报告不将尚未实现的完整多租户、对象存储和高级流程节点误记为通过。
