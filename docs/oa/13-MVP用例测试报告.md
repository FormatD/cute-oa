...
| TC-PROCESS-004 | 节点 SLA 配置持久化、管理员发布前试算、普通员工越权拒绝、审批人缺失转流程管理员、连续重复节点跳过/阻断及非连续重复保留 | Playwright + `Oa.Postgres.Tests` | 通过 |
...
| TC-FLOW-SLA-001 | 首节点立即起算、后续节点延迟激活；同意激活下一节点，驳回取消剩余节点，事项中心返回截止时间 | `Oa.Postgres.Tests` | 通过 |
| TC-FLOW-SLA-002 | 临期、逾期、升级通知及正确接收人；重复扫描数据库去重；活动待办阻止审批人停用 | `Oa.Postgres.Tests` | 通过 |
...
- 会签、或签、加签、按法定工作日计时的审批 SLA 与多级升级链；当前通用审批和员工生命周期任务均已具备自然时长临期、逾期和单级升级提醒；
