...
<template>
  ...
  <section v-if="editingId" class="panel user-editor"><div class="section-title"><div><p class="eyebrow">USER EDITOR</p><h2>{{ editingId === 'new' ? '新建用户并建档' : `编辑用户 · ${editingId}` }}</h2></div><button class="secondary" @click="editingId = ''">关闭</button></div><form class="user-form" @submit.prevent="save"><label>用户 ID<input v-model="form.id" maxlength="64" :disabled="editingId !== 'new'" placeholder="例如 u-zhou"></label><label>姓名<input v-model="form.name" maxlength="64"></label><label>所属部门<select v-model="form.departmentId" @change="departmentChanged"><option v-for="department in organization.departments" :key="department.id" :value="department.id">{{ department.name }}</option></select></label><label>主岗位<select v-model="form.positionId"><option value="">未分配</option><option v-for="position in availablePositions" :key="position.id" :value="position.id">{{ position.name }}</option></select><small>岗位描述组织职责，不授予系统权限。</small></label><label>直属上级<UserSelect v-model="form.managerId" :exclude-user-ids="form.id ? [form.id] : []" placeholder="选择直属上级（无则留空）" /></label><label>社会工龄（年）<input v-model.number="form.cumulativeWorkYears" type="number" min="0" max="60"><small>未填写累计工作起始日时作为兼容值。</small></label><label v-if="editingId !== 'new'">账号状态<select v-model="form.status"><option value="ACTIVE">启用</option><option value="DISABLED">停用</option></select></label><template v-if="editingId === 'new'"><label>工号<input v-model.trim="form.employeeNumber" maxlength="32" placeholder="例如 EMP-2026-001"></label><label>入职日期<input v-model="form.hireDate" type="date"></label><label>用工类型<select v-model="form.employmentType"><option value="FULL_TIME">全职</option><option value="PART_TIME">兼职</option><option value="INTERN">实习</option><option value="CONTRACTOR">外包</option></select></label><label>人事状态<select v-model="form.personnelStatus"><option value="ACTIVE">在职</option><option value="PROBATION">试用</option></select></label><label v-if="form.personnelStatus === 'PROBATION'">试用期结束<input v-model="form.probationEndDate" type="date"></label><label>累计工作起始日<input v-model="form.cumulativeWorkStartDate" type="date"><small>用于精确计算法定年假；不确定时可暂不填写。</small></label><label>临时密码<input v-model="form.password" type="password" minlength="12" maxlength="128" autocomplete="new-password"><small>12–128 位，须包含大小写字母、数字和特殊字符；员工首次登录仍必须设置新的正式密码。</small></label></template><fieldset class="wide role-selector"><legend>安全角色与权限</legend><label v-for="role in identity.roles" :key="role.code"><input v-model="form.roles" type="checkbox" :value="role.code"><span><strong>{{ role.name }}</strong><small>{{ role.permissions.map(permission => permissionLabels[permission] ?? permission).join('、') || '基础员工权限' }}</small></span></label></fieldset><div class="form-actions wide"><button :disabled="identity.saving" type="submit">{{ identity.saving ? '保存中…' : '保存用户' }}</button></div></form>
    <form v-if="editingId !== 'new'" class="password-reset" @submit.prevent="submitPasswordReset"><div><strong>重置临时密码</strong><small>须为 12 位四类字符强密码；重置后撤销全部会话并要求用户下次登录设置正式密码。</small></div><input v-model="resetPassword" type="password" minlength="12" maxlength="128" autocomplete="new-password" placeholder="输入临时密码"><button class="secondary" :disabled="identity.saving || resetPassword.length < 12" type="submit">重置密码</button></form>
    <div v-if="editingId !== 'new' && identity.users.find(user => user.id === editingId)?.mfaEnabled" class="password-reset"><div><strong>重置多因素认证</strong><small>用于用户遗失身份验证器和全部恢复码；会撤销该用户所有登录会话。</small></div><span class="mfa-admin-state">当前已启用</span><button class="danger-outline" :disabled="identity.saving" type="button" @click="submitMfaReset">重置 MFA</button></div>
  </section>
</template>
