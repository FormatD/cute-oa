...
onMounted(async () => { await Promise.all([organization.loadOrganization(), contracts.loadContracts(), effectiveConfig.load()]) })
</script>
...
<template>
  ...
  <OaDialog :open="createOpen" title="新建劳动合同" description="先保存草稿；填写签订日期并上传签署件后，可在详情页激活。" submit-label="保存草稿" :busy="contracts.saving || uploading" :submit-disabled="!!effectiveConfig.error" @close="createOpen = false" @submit="save">
    ...
    <div class="dialog-grid">
      <label class="dialog-field">员工<select v-model="form.userId" @change="employeeChanged"><option v-for="employee in organization.directoryEmployees" :key="employee.id" :value="employee.id">{{ employee.name }} · {{ employee.departmentName }}</option></select></label>
      <label class="dialog-field">合同类型<select v-model="form.contractType" @change="typeChanged"><option v-for="ct in effectiveConfig.contractTypes" :key="ct.code" :value="ct.code as EmploymentContractType">{{ ct.name }}</option></select></label>
      <label class="dialog-field">签订日期<input v-model="form.signedDate" type="date" :max="today"></label>
      <label class="dialog-field">开始日期<input v-model="form.startDate" type="date"></label>
      <label v-if="form.contractType !== 'OPEN_ENDED'" class="dialog-field">结束日期<input v-model="form.endDate" type="date"></label>
      <label class="dialog-field">工作地点<input v-model="form.workLocation" maxlength="100"></label>
      <label class="dialog-field">合同岗位<input v-model="form.positionName" maxlength="100"></label>
      <label class="dialog-field">试用期开始<input v-model="form.probationStartDate" type="date"></label>
      <label class="dialog-field">试用期结束<input v-model="form.probationEndDate" type="date"></label>
      <label v-if="form.contractType === 'PROJECT_BASED'" class="dialog-field">任务说明<textarea v-model="form.projectDescription" maxlength="500"></textarea></label>
    </div>
    <label class="dialog-field">签署附件（最多 10 个）<input type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" :disabled="uploading || form.attachments.length >= 10" @change="upload"><small>{{ uploading ? '正在上传…' : `已关联 ${form.attachments.length} 个附件` }}</small></label>
    <label class="dialog-field">备注<textarea v-model="form.notes" maxlength="1000"></textarea></label>
    <label class="dialog-field">建档说明<textarea v-model="form.changeReason" maxlength="500"></textarea></label>
  </OaDialog>
</template>
