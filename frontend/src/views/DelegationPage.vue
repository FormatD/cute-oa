...
const candidates = computed(() => workspace.employees.filter(employee => employee.id !== auth.currentUserId))
const businessLabel = (type: string) => ({ All: '全部业务', Leave: '请假', Expense: '报销', Travel: '出差' }[type] ?? type)
...
<template>
  ...
  <section class="panel"><div class="section-title"><div><p class="eyebrow">NEW RULE</p><h2>新建委托</h2></div></div><form class="delegation-form" @submit.prevent="delegation.submit"><label>代办人<select v-model="delegation.form.delegateId"><option value="">请选择</option><option v-for="employee in candidates" :key="employee.id" :value="employee.id">{{ employee.name }} · {{ employee.role }}</option></select></label><label>业务范围<select v-model="delegation.form.businessType"><option value="All">全部业务</option><option value="Leave">请假</option><option value="Expense">报销</option><option value="Travel">出差</option></select></label><label>开始时间<input v-model="delegation.form.startAt" type="datetime-local"></label><label>结束时间<input v-model="delegation.form.endAt" type="datetime-local"></label><label class="wide">委托原因<textarea v-model="delegation.form.reason" maxlength="200" placeholder="例如：出差期间由同事代办审批"></textarea></label><div class="form-actions wide"><button :disabled="delegation.saving" type="submit">{{ delegation.saving ? '保存中…' : '创建委托' }}</button></div></form></section>
