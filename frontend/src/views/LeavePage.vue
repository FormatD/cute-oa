...
<template>
  ...
  <form class="filter-bar" @submit.prevent="search"><input v-model="leave.leaveFilters.keyword" placeholder="单号、事由或申请人"><select v-model="leave.leaveFilters.status"><option value="">全部状态</option><option value="0">草稿</option><option value="1">审批中</option><option value="2">已驳回</option><option value="3">已完成</option><option value="4">已撤回</option></select><select v-model="leave.leaveFilters.applicantId"><option value="">全部申请人</option><option v-for="employee in workspace.employees" :key="employee.id" :value="employee.id">{{ employee.name }}</option></select><label>开始<input v-model="leave.leaveFilters.startDate" type="date"></label><label>结束<input v-model="leave.leaveFilters.endDate" type="date"></label><button type="submit">查询</button><button class="secondary" type="button" @click="resetFilters">重置</button></form>
  <section class="panel"><div class="section-title"><div><p class="eyebrow">LEAVE</p><h2>请假申请</h2></div><button @click="leave.showForm = !leave.showForm">{{ leave.showForm ? '收起表单' : '新建请假' }}</button></div>
    <form v-if="leave.showForm" class="leave-form" @submit.prevent="app.submitLeave"><label>假别<select v-model="leave.form.type"><option value="Annual">年假</option><option value="Personal">事假</option><option value="Sick">病假</option><option value="CompTime">调休</option></select></label><label>开始日期<input v-model="leave.form.startDate" type="date"></label><label>开始时段<select v-model="leave.form.startPeriod"><option value="FullDay">全天</option><option value="Morning">上午</option><option value="Afternoon">下午</option></select></label><label>结束日期<input v-model="leave.form.endDate" type="date"></label><label>结束时段<select v-model="leave.form.endPeriod"><option value="FullDay">全天</option><option value="Morning">上午</option><option value="Afternoon">下午</option></select></label><label class="wide">请假事由<textarea v-model="leave.form.reason" maxlength="500" placeholder="请填写请假原因"></textarea></label><fieldset class="wide copy-selector"><legend>抄送人（流程完成或撤回后通知）</legend><label v-for="employee in workspace.employees.filter(item => item.id !== auth.currentUserId)" :key="employee.id"><input v-model="leave.form.copyRecipientIds" type="checkbox" :value="employee.id">{{ employee.name }} · {{ employee.role }}</label></fieldset><label class="wide">证明材料<input accept=".pdf,.jpg,.jpeg,.png,.xls,.xlsx,.doc,.docx" multiple type="file" @change="selectAttachments"><small>支持 PDF、图片和 Office 文档，单个文件不超过 20MB。</small></label><div v-if="leave.form.attachments.length" class="wide attachment-list">已上传 {{ leave.form.attachments.length }} 个附件</div><div class="wide form-actions"><button :disabled="ui.submitting" type="submit">{{ ui.submitting ? '提交中…' : '保存并提交' }}</button></div></form>
