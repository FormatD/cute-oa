...
<template>
  ...
  <section v-if="attendance.monthLock" class="attendance-lock" :class="{ locked: isLocked }"><strong>{{ attendance.filters.month.slice(0, 7) }} {{ isLocked ? '已封账' : '未封账' }}</strong><span v-if="isLocked">{{ attendance.monthLock.lockedByName }} 于 {{ new Date(attendance.monthLock.lockedAt!).toLocaleString('zh-CN') }} 完成封账 · {{ attendance.monthLock.lockReason }}</span><span v-if="isLocked && attendance.monthLock.snapshotSequence">第 {{ attendance.monthLock.snapshotSequence }} 版不可变快照 · {{ attendance.monthLock.snapshotRowCount }} 人 · SHA-256 {{ attendance.monthLock.snapshotHash?.slice(0, 12) }}…</span><span v-else-if="!isLocked && attendance.monthLock.unlockedAt">最近由 {{ attendance.monthLock.unlockedByName }} 于 {{ new Date(attendance.monthLock.unlockedAt).toLocaleString('zh-CN') }} 解封 · {{ attendance.monthLock.unlockReason }}</span><span v-else-if="!isLocked">HR 完成异常申诉后可封账，封账后禁止导入、修改和申诉。</span></section>
