...
async function openCopy(item: FlowCopy) { await workflow.markCopyRead(item); if (item.readAt) await router.push(`/${item.businessType === 'Leave' ? 'leave' : item.businessType === 'Expense' ? 'expense' : 'travel'}/${item.businessId}`) }
...
<template>
  <template v-if="workspace.summary">
    ...
    <section class="cards"><article><span class="card-icon blue">✓</span><div><small>待我审批</small><strong>{{ workspace.summary.pendingTaskCount }}</strong><em>待处理事项</em></div></article><article><span class="card-icon green">◷</span><div><small>我的年假</small><strong>{{ workspace.summary.leaveBalance.available }}<sup>天</sup></strong><em>已冻结 {{ workspace.summary.leaveBalance.frozen }} 天</em></div></article><article><span class="card-icon orange">▤</span><div><small>待我阅读</small><strong>{{ workspace.summary.pendingReadCount }}</strong><em>未读抄送事项</em></div></article><article class="record-link" @click="router.push('/hr/contracts')"><span class="card-icon orange">▧</span><div><small>合同风险</small><strong>{{ workspace.summary.contractRiskCount }}</strong><em>待处理或待确认</em></div></article></section>
