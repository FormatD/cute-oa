...
const form = reactive({ code: '', name: '', permissions: [] as string[], leaveDataScope: 'SELF' as DataScope, expenseDataScope: 'SELF' as DataScope, travelDataScope: 'SELF' as DataScope, purchaseDataScope: 'SELF' as DataScope, personnelDataScope: 'SELF' as DataScope, attendanceDataScope: 'SELF' as DataScope, contractDataScope: 'SELF' as DataScope })
...
function openCreate() {
  editing.value = false
  Object.assign(form, { code: '', name: '', permissions: [], leaveDataScope: 'SELF', expenseDataScope: 'SELF', travelDataScope: 'SELF', purchaseDataScope: 'SELF', personnelDataScope: 'SELF', attendanceDataScope: 'SELF', contractDataScope: 'SELF' })
...
async function saveRole() {
  ...
  const payload = { code: form.code.trim(), name: form.name.trim(), permissions: [...form.permissions], dataScopes: { Leave: form.leaveDataScope, Expense: form.expenseDataScope, Travel: form.travelDataScope, Purchase: form.purchaseDataScope, Personnel: form.personnelDataScope, Attendance: form.attendanceDataScope, Contract: form.contractDataScope } }
...
<template>
  ...
  <section class="panel">
    ...
    <template v-else><div class="table-wrap"><table class="data-table role-table"><thead><tr><th>角色</th><th>类型</th><th>已分配用户</th><th>数据范围</th><th>操作权限</th><th class="action-cell">操作</th></tr></thead><tbody><tr v-for="role in identity.roles" :key="role.code"><td><strong>{{ role.name }}</strong><small>{{ role.code }}</small></td><td><em :class="{ archived: !role.isSystem }">{{ role.isSystem ? '系统预置' : '自定义' }}</em></td><td>{{ role.userCount }} 人</td><td><small>请假：{{ scopeLabel(role.dataScopes?.Leave ?? 'SELF') }}</small><small>报销：{{ scopeLabel(role.dataScopes?.Expense ?? 'SELF') }}</small><small>出差：{{ scopeLabel(role.dataScopes?.Travel ?? 'SELF') }}</small><small>采购：{{ scopeLabel(role.dataScopes?.Purchase ?? 'SELF') }}</small><small>人事：{{ scopeLabel(role.dataScopes?.Personnel ?? 'SELF') }}</small><small>考勤：{{ scopeLabel(role.dataScopes?.Attendance ?? 'SELF') }}</small><small>合同：{{ scopeLabel(role.dataScopes?.Contract ?? 'SELF') }}</small></td><td><span v-if="!role.permissions.length" class="muted">基础业务权限</span><span v-for="permission in role.permissions" :key="permission" class="role-chip">{{ identity.permissions.find(item => item.code === permission)?.name ?? permission }}</span></td><td class="task-actions"><button class="secondary" @click="openEdit(role)">编辑</button><button v-if="!role.isSystem" class="danger-outline" :disabled="role.userCount > 0" :title="role.userCount ? '请先从用户移除此角色' : '删除角色'" @click="deleteTarget = role">删除</button></td></tr></tbody></table></div></template>
  </section>
  ...
  <OaDialog :open="editorOpen" :title="editing ? '编辑角色' : '新建角色'" :description="editing ? '权限变化后，使用该角色的有效会话将立即失效。' : '创建后可在用户管理中分配给员工。'" submit-label="保存角色" :busy="identity.saving" @close="editorOpen = false" @submit="saveRole">
    ...
    <div class="dialog-grid"><label class="dialog-field">请假数据范围<select v-model="form.leaveDataScope"><option v-for="option in dataScopeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>直属上下级关系不受此范围限制。</small></label><label class="dialog-field">报销数据范围<select v-model="form.expenseDataScope"><option v-for="option in dataScopeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>跨用户范围需要对应的范围查看权限。</small></label><label class="dialog-field">出差数据范围<select v-model="form.travelDataScope"><option v-for="option in dataScopeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>跨用户范围需要对应的范围查看权限。</small></label><label class="dialog-field">采购数据范围<select v-model="form.purchaseDataScope"><option v-for="option in dataScopeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>采购执行权限仍受所选数据范围约束。</small></label><label class="dialog-field">人事档案数据范围<select v-model="form.personnelDataScope"><option v-for="option in dataScopeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>上级仍可查看管理链下属。</small></label><label class="dialog-field">考勤数据范围<select v-model="form.attendanceDataScope"><option v-for="option in dataScopeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>员工本人和管理链上级始终可查看。</small></label><label class="dialog-field">劳动合同数据范围<select v-model="form.contractDataScope"><option v-for="option in dataScopeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>合同属于敏感数据，上级不会自动获得下属查看权。</small></label></div>
