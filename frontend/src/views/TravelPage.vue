...
<template>
  ...
  <form class="filter-bar" @submit.prevent="search"><input v-model="travel.travelFilters.keyword" placeholder="单号、地点、事由或申请人"><select v-model="travel.travelFilters.status"><option value="">全部状态</option><option value="0">草稿</option><option value="1">审批中</option><option value="2">已驳回</option><option value="3">已批准</option><option value="4">已撤回</option></select><select v-model="travel.travelFilters.applicantId"><option value="">全部申请人</option><option v-for="employee in workspace.employees" :key="employee.id" :value="employee.id">{{ employee.name }}</option></select><label>开始<input v-model="travel.travelFilters.startDate" type="date"></label><label>结束<input v-model="travel.travelFilters.endDate" type="date"></label><button type="submit">查询</button><button class="secondary" type="button" @click="resetFilters">重置</button></form>
  <section class="panel"><div class="section-title"><div><p class="eyebrow">TRAVEL REQUESTS</p><h2>出差申请</h2></div><button @click="travel.showTravelForm = !travel.showTravelForm">{{ travel.showTravelForm ? '收起表单' : '新建出差' }}</button></div>
    <form v-if="travel.showTravelForm" class="leave-form" @submit.prevent="app.submitTravel"><label class="wide">出差事由<textarea v-model="travel.travelForm.purpose" maxlength="500" placeholder="说明出差目的和预期成果"></textarea></label><label>预估预算（元）<input v-model="travel.travelForm.estimatedBudget" min="0" max="10000000" step="0.01" type="number"></label><fieldset class="wide copy-selector"><legend>同行人（可选）</legend><label v-for="employee in workspace.employees.filter(item => item.id !== auth.currentUserId)" :key="employee.id"><input v-model="travel.travelForm.companionIds" type="checkbox" :value="employee.id">{{ employee.name }} · {{ employee.departmentName }}</label></fieldset>
      <fieldset class="wide itinerary-editor"><legend>行程明细</legend><div v-for="(item, index) in travel.travelForm.itinerary" :key="index" class="itinerary-row"><label>目的地<input v-model="item.destination" maxlength="100" placeholder="例如 上海"></label><label>开始日期<input v-model="item.startDate" type="date"></label><label>结束日期<input v-model="item.endDate" type="date"></label><label>交通方式<select v-model="item.transportation"><option v-for="transport in transports" :key="transport">{{ transport }}</option></select></label><label class="wide">行程任务<input v-model="item.purpose" maxlength="300" placeholder="例如 客户需求确认与方案汇报"></label><button class="secondary" type="button" :disabled="travel.travelForm.itinerary.length === 1" @click="removeItinerary(index)">删除行程</button></div><button class="secondary" type="button" :disabled="travel.travelForm.itinerary.length >= 20" @click="addItinerary">＋ 添加行程</button></fieldset>
      <fieldset class="wide copy-selector"><legend>抄送人（审批完成或撤回后通知）</legend><label v-for="employee in workspace.employees.filter(item => item.id !== auth.currentUserId)" :key="employee.id"><input v-model="travel.travelForm.copyRecipientIds" type="checkbox" :value="employee.id">{{ employee.name }} · {{ employee.role }}</label></fieldset><label class="wide">相关附件<input accept=".pdf,.jpg,.jpeg,.png,.xls,.xlsx,.doc,.docx" multiple type="file" @change="selectAttachments"><small>可上传邀请函、会议通知或行程依据，单个文件不超过 20MB。</small></label><div v-if="travel.travelForm.attachments.length" class="wide attachment-list">已上传 {{ travel.travelForm.attachments.length }} 个附件</div><div class="wide form-actions"><button :disabled="travel.travelSubmitting" type="submit">{{ travel.travelSubmitting ? '提交中…' : '保存并提交' }}</button></div></form>
