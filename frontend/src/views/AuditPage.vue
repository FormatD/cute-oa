...
<template>
  ...
  <section class="panel"><form class="filter-bar" @submit.prevent="audit.search"><input v-model="audit.filters.keyword" placeholder="动作、摘要或资源 ID"><select v-model="audit.filters.actorId"><option value="">全部操作人</option><option v-for="employee in employeeDirectory.employees" :key="employee.id" :value="employee.id">{{ employee.name }}</option></select><select v-model="audit.filters.resourceType"><option value="">全部资源</option><option>LeaveRequest</option><option>ExpenseClaim</option><option>TravelRequest</option><option>PurchaseRequest</option><option>PersonnelProfile</option><option>AttendanceShift</option><option>AttendanceRecord</option><option>AttendanceAppeal</option><option>EmploymentContract</option><option>File</option><option>WorkCalendar</option></select><label>开始<input v-model="audit.filters.startDate" type="date"></label><label>结束<input v-model="audit.filters.endDate" type="date"></label><button type="submit">查询</button><button class="secondary" type="button" @click="audit.resetFilters">重置</button></form>
