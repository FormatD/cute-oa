...
export type OverBudgetSnapshot = { isOverBudget: boolean; requestedAmount: number; availableAmount: number; overBudgetAmount: number; department?: string | null; category?: string | null; policyCode: string; resolvedAt: string }
export type PurchaseReconciliationItem = { id: string; number: string; applicantName: string; departmentName: string; estimatedTotal: number; orderAmount: number; orderNumber?: string | null; supplier?: string | null; acceptedAmount: number; paidAmount: number; remainingAmount: number; status: string; createdAt: string }
