...
export type DetailView = { module: 'leave' | 'expense' | 'travel' | 'purchase' | 'notification' | 'calendar'; id: string }
...
export type ExpenseForm = { category: string; amount: string; expenseDate: string; description: string; receiptNumber: string; attachments: string[]; copyRecipientIds: string[]; travelRequestId: string }
...
export type ProcessRoute = { id: string; sequence: number; maxValue: number | null; approverKeys: string[] }
export type ProcessDefinition = { id: string; code: string; name: string; businessType: 'Leave' | 'Expense' | 'Travel' | 'Purchase'; version: number; status: number; routes: ProcessRoute[]; createdBy: string; createdAt: string; publishedBy?: string | null; publishedAt?: string | null; priority: number; departmentIds: string[]; leaveTypes: string[] }
export type UpdateProcessDefinition = { name: string; routes: Array<{ maxValue: number | null; approverKeys: string[] }>; priority: number; departmentIds: string[]; leaveTypes: string[] }
export type CreateProcessDefinition = UpdateProcessDefinition & { code: string; businessType: 'Leave' | 'Expense' | 'Travel' | 'Purchase' }
export type FlowDelegation = { id: string; ownerId: string; ownerName: string; delegateId: string; delegateName: string; businessType: 'All' | 'Leave' | 'Expense' | 'Travel' | 'Purchase'; startAt: string; endAt: string; reason: string; status: number; isEffective: boolean; createdAt: string; cancelledAt?: string | null }
export type FlowCopy = { id: string; businessType: 'Leave' | 'Expense' | 'Travel' | 'Purchase'; businessId: string; businessNumber: string; applicantName: string; title: string; status: string; availableAt: string; readAt?: string | null }
export type DataScope = 'SELF' | 'DEPARTMENT' | 'DEPARTMENT_AND_CHILDREN' | 'COMPANY'
export type SecurityRole = { code: string; name: string; isSystem: boolean; userCount: number; permissions: string[]; dataScopes: Record<'Leave' | 'Expense' | 'Travel' | 'Purchase' | 'Personnel' | 'Attendance' | 'Contract', DataScope> }
export type PermissionDefinition = { code: string; name: string; description: string }
export type CreateSecurityRole = { code: string; name: string; permissions: string[]; dataScopes: Record<'Leave' | 'Expense' | 'Travel' | 'Purchase' | 'Personnel' | 'Attendance' | 'Contract', DataScope> }
export type UpdateSecurityRole = { code: string; name: string; permissions: string[]; dataScopes: Record<'Leave' | 'Expense' | 'Travel' | 'Purchase' | 'Personnel' | 'Attendance' | 'Contract', DataScope> }
...
export type EmploymentContractType = 'FIXED_TERM' | 'OPEN_ENDED' | 'PROJECT_BASED'
